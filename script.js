...
const account1 = {
  ...
  transactionsDates: [
    ...
    '2022-08-05T11:42:26.371Z',
    '2022-08-03T07:43:59.331Z',
    '2022-08-07T15:21:20.814Z',
  ],
  currency: 'USD',
  locale: 'en-US',
};
...
const formatTransactionDate = (date, locale) => {
  ...
  if (daysPassed >= 5 && daysPassed <= 7) return `${daysPassed} днів назад`;
  else {
...
const displayTotal = account => {
  ...
  const interestTotal = account.transactions
    .filter(trans => trans > 0)
    .map(depo => (depo * account.interest) / 100)
...
const startLogOutTimer = () => {
  const logOutTimerCallback = () => {
    //В кожному виклику показувати залишок часу в UI
    const minutes = String(Math.trunc(time / 60)).padStart(2, '0');
    const seconds = String(time % 60).padStart(2, '0');

    labelTimer.textContent = `${minutes}:${seconds}`;

    // Після закінчення часу зупинити таймер і вийти із додатку

    if (time === 0) {
      clearInterval(logOutTimer);
      containerApp.style.opacity = 0;
      labelWelcome.textContent = 'Увійдіть у свій аккаунт';
    }
...
btnLogin.addEventListener('click', e => {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    labelWelcome.textContent = `Раді, що ви знову із нами, ${
...
//  175. Array.from() example в веб приложении
...
// 186. Используем Оператор Остаток В Приложении
